...
def train(train_loader, model, loss_fn, loss_cont_fn, optimizer, epoch, args, tb_logger, start_upd_prot=False):
    ...
    for i, (images_w, images_s, labels, true_labels, index) in enumerate(train_loader):
        ...
        #loss = loss_cls + args.loss_weight * ((1-args.hyper_distance)*loss_cont_angle +  args.hyper_distance*loss_cont_dist)# ---------we want to print this this will give us per batch loss -----
        loss = loss_cls + args.loss_weight * (loss_cont_angle +  args.hyper_distance*loss_cont_dist)
